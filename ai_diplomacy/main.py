...
def main():
    ...
    for power_name, model_id in game.power_model_map.items():
        if not game.powers[
            power_name
        ].is_eliminated():  # Only create for active powers initially
            try:
                ...
                try:
                    ...
                    logger.error(
                        f"Failed to initialize agent state for {power_name}: {e}",
                        exc_info=True,
                    )
                    # Decide if we should continue without initialized state or exit? For now, continue.
            except Exception as e:
                logger.error(
                    f"Failed to initialize agent for {power_name} with model {model_id}: {e}"
                )
        else:
            logger.info(
                f"Skipping agent initialization for eliminated power: {power_name}"
            )
    # =======================================
    ...
    while not game.is_game_done:
        ...
        with concurrent.futures.ThreadPoolExecutor(
            ...
            for power_name, _ in active_powers:
                ...
                if power_name not in agents:
                    logger.warning(
                        f"Agent not found for active power {power_name}. Skipping order generation."
                    )
                    continue
                ...
                if not possible_orders:
                    logger.debug(
                        f"No orderable locations for {power_name}; skipping order generation."
                    )
                    continue
            ...
            for pwr, pwr_orders in orders_dict.items():
                for order_str in pwr_orders:
                    ...
                    if not results_list or all(
                        ...
                        fail_dict.setdefault(pwr, []).append(
                            f"{order_str} - {', '.join(str(r) for r in results_list)}"
                        )
                    else:
                        other_dict.setdefault(pwr, []).append(order_str)
        ...
        if not game.is_game_done:
            ...
            phase_summary = game.phase_summaries.get(
                completed_phase_name, "Summary not found"
            )
                ...
                if power_name in agents:
                    try:
                        ...
                        agents[power_name].analyze_phase_and_update_state(
                            game, current_board_state, phase_summary, game_history
                        )
                        logger.info(f"Agent state analysis completed for {power_name}.")
                    except Exception as exc:
                        logger.error(
                            f"Agent state analysis failed for {power_name}: {exc}",
                            exc_info=True,
                        )
                else:
                    logger.warning(
                        f"Skipping state analysis for non-agent power {power_name}"
                    )
            ...
            if phase.name == current_short_phase:
                for msg in phase.messages:
                    try:
                        ...
                        if not any(
                            m.sender == msg.sender
                            and m.recipient == msg.recipient
                            and m.message == msg.content
        ...
        logger.info(
            f"Starting state update analysis for completed phase {completed_phase_name}..."
        )

        # Ensure phase summary exists (it should be generated by the callback during game.process)
        phase_summary = game.phase_summaries.get(
            completed_phase_name, f"Summary for {completed_phase_name} not found."
        )
        ...
        active_agent_powers = [
            p for p in active_powers if p[0] in agents
        ]  # Filter for powers with active agents

        if active_agent_powers:  # Only run if there are agents to update
            with concurrent.futures.ThreadPoolExecutor(
                max_workers=len(active_agent_powers)
            ) as executor:
                analysis_futures = {}
                ...
                for future in concurrent.futures.as_completed(analysis_futures):
                    ...
                    try:
                        ...
                        logger.debug(
                            f"State analysis completed successfully for {power_name}."
                        )
                    except Exception as e:
                        logger.error(
                            f"Error during state analysis for {power_name}: {e}"
                        )

            logger.info(
                f"Completed state update analysis for phase {completed_phase_name}."
            )
        else:
            logger.info(
                f"No active agents found to perform state update analysis for phase {completed_phase_name}."
            )
        # ===============================================
    ...
    logger.info(
        f"Game has {len(game.phase_summaries)} phase summaries: {list(game.phase_summaries.keys())}"
    )
    ...
    logger.info(
        f"Added phase_summaries to saved game with {len(game.phase_summaries)} phases"
    )
    ...
    logger.info(
        f"Added summaries to {summary_phases_count}/{len(saved_game['phases'])} phases in the export"
    )
